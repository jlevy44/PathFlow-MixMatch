...
def displace_image(img, displacement):
    ...
    for i in range(3):
        ...
        im=al.utils.image.create_tensor_image_from_itk_image(im, dtype=th.float32, device='cuda')
...
def affine_register(im1, im2, iterations=1000, lr=0.01, transform_type='similarity'):
...
def get_loss(im1,im2):
    ...
    _, _, phi,loss=affine_register(np.uint8(im1), np.uint8(im2), 100, lr=0.01, transform_type='rigid')
    return loss

def rotate_detector(im1,im2):
    angles={}
    for angle in [0, 90, 180, 270]:
        im_test=rotate(im2,angle, resize=True, center=None, order=1, mode='constant', cval=255., clip=False, preserve_range=False)
        angles[angle]=get_loss(im1,im_test)
    return angles

def correct_rotation(im1, im2, scaling_factor=4):
    ...
    with suppress_stdout():
        rotation_loss_dict=rotate_detector(im1_small,im2_small)
...
def register_images_(npy1='A.npy', npy2='B.npy', connectivity=8, apply_watershed=False, overwrite=False, min_object_size=50000, clip=True, fix_rotation=True, mult_factor=2.0, scaling_factor=4, output_dir='practice_reg_results/images', properties=['area','convex_area','eccentricity','major_axis_length','minor_axis_length','inertia_tensor','inertia_tensor_eigvals','perimeter','orientation']):
    ...
    if fix_rotation:
        ...
        im2=correct_rotation(im1, im2, scaling_factor=scaling_factor)
    ...
    for idx,(i,j) in enumerate(sections):
        ...
        if os.path.exists(npy_out1)==False or os.path.exists(npy_out1)==False or overwrite:
            ...
            try:
                ...
                img1=np.uint8(rotate(img, -props.loc[i,'orientation']*180./(np.pi), resize=True, center=None, order=1, mode='constant', cval=1., clip=clip, preserve_range=False)*255.)
                ...
                img2=np.uint8(rotate(img, -props2.loc[j,'orientation']*180./(np.pi), resize=True, center=None, order=1, mode='constant', cval=1., clip=clip, preserve_range=False)*255.)
                ...
                with suppress_stdout():
                    new_img=displace_image(img2,affine_register(img1, img2)[0]) # new tri, output he as well
