...
def img2h(img):
    hed=rgb2hed(img)
    return rescale_intensity(hed[:, :, 0], out_range=(0, 1))
...
#     assert use_mask==False, "Masking not implemented"
    if use_mask: assert transform_type in ['similarity', 'affine', 'rigid']
    ...
    if use_mask and mask1 is None and mask2 is None:
        from pathflowai.utils import generate_tissue_mask
        mask1,mask2=th.tensor(generate_tissue_mask(im1).astype(float)),th.tensor(generate_tissue_mask(im2).astype(float))
    ...
    if not moving_image:
        if isinstance(im2, np.ndarray):
            moving_image = al.utils.image.create_tensor_image_from_itk_image(sitk.GetImageFromArray(cv2.cvtColor(im2, cv2.COLOR_BGR2GRAY) if not use_hematoxylin else img2h(im2)), dtype=th.float32, device='cpu')
    ...
    if use_mask:
        mask1,mask2=Image(mask1, fixed_image.size, fixed_image.spacing, fixed_image.origin),Image(mask2, moving_image.size, moving_image.spacing, moving_image.origin)
    ...
    for level, (mov_im_level, fix_im_level) in enumerate(zip(moving_image_pyramid, fixed_image_pyramid)):
        ...
        if use_mask:
            mask1 = mask1.to(dtype=th.float32, device=device)
            mask2 = mask2.to(dtype=th.float32, device=device)

        image_loss = loss_fns[loss_fn](fix_im_level, mov_im_level, fixed_mask=mask1, moving_mask=mask2)
    ...
    if transform_type in ['similarity', 'affine', 'rigid']:
        transformation_param = list(transformation.named_parameters())#._phi_z
